...
for i in range(no_runs): # [0; no_runs - 1]
    ...
    "UNION" \
    "(SELECT * FROM store.payments WHERE channel = 'Physical - Suburban' ORDER BY RAND() LIMIT 0,667)" \
    "UNION" \
    "(SELECT * FROM store.payments WHERE channel = 'Online' ORDER BY RAND() LIMIT 0,667)")
